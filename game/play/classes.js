...
class Texture {
    ...
    slice() {
        if (extensiveLogging) console.log("Slicing image: " + this.source);
        const size = 16;
        let img = new Image();
        img.src = "imgs/" + this.source;
        img.addEventListener("load", () => {
            if (extensiveLogging) console.log("Image loaded for slicing: " + this.source);
            ...
            if (extensiveLogging) console.log(`Image sliced into ${this.pics.length} part${this.pics.length > 1 ? "s" : ""}: ${this.source}`);
            // this.onslicedone();
        });
    }
...
class TextureAnimation {
    ...
    playAnimation(times=1, animationID=this.animationID) {
        ...
        setTimeout(() => {
            ...
            if (extensiveLogging) console.log("ye - " + animationID);
            this.nextFrame();
...
class Effect {
    constructor(name="", miliseconds=0, priority=0, func=()=>{}, aftereffect=()=>{}, delay=0) {
        ...
        this.parent = null;
        this.stoppedFor = 0;
        if (extensiveLogging) console.log(`Effect: ${name} added with duration: ${miliseconds} with delay: ${delay}`);
        this.callUpdate();
    }
    get timeleft() {
    ...
    update() {
        ...
        if (timePassed < 0) {
            if (extensiveLogging) console.log("waitin'");
            return this.callUpdate();
        }
        if (timePassed >= this.miliseconds) {
            if (extensiveLogging) console.log(this);
            if (!(this.name in this.effectReference)) console.warn("Tried removing effect that was not there: " + this.name + " ; keys are: " + Object.keys(this.effectReference));
                ...
                this.aftereffect();
                if (extensiveLogging) console.log(`Effect: '${this.name}' removed, it was active for ${timePassed}ms ; target was: ${this.miliseconds} ; it was stopped for ${this.stoppedFor}ms`)
    ...
    extendEffect(miliseconds) {
        if (extensiveLogging) console.log(`Effect '${this.name}' extended by ${miliseconds}ms ; ${this.miliseconds} -> ${this.miliseconds + miliseconds} ; Time left: ${(this.miliseconds + miliseconds) - (Date.now() - this.startTime)}`);
        this.miliseconds += miliseconds;
    }
}